...
exports.launchFilter = function(mcast, port, samplerate, channels, type){
	...
	switch(type){
		case 'ebur128':
			filter = 'ffplay -hide_banner -loglevel error -window_title "ebur128" -f lavfi "amovie=/dev/stdin,ebur128=video=1:size=1280x720:meter=9:gauge=shortterm:scale=absolute[v-ebur][a];[a]showvolume=b=4:w=720:h=30:t=0:dm=3,transpose=2[v-sv];[v-ebur][v-sv]hstack=2[out0]"';
		break;
		case 'spectrum':
